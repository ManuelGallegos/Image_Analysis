...
namespace ImageAnalysis
{
    ...
    {
        public static async Task AnalyzeImageFromUrl(ComputerVisionClient client, string imageUrl)
        {
            ...
            Microsoft.Azure.CognitiveServices.Vision.ComputerVision.Models.ImageAnalysis analysisData = await client.AnalyzeImageAsync(imageUrl, featuresToBeAnalyzed);

            //Sumarizes the image content
            Console.WriteLine("Analysis at a glance: ");

            foreach(var caption in analysisData.Description.Captions)
            {
                Console.Write($"{caption.Text} with confidence {caption.Confidence}");
            }
            ...
            Console.WriteLine("Image color name");
            ...
            Console.WriteLine("Cominant background color: " + analysisData.Color.DominantColorBackground);
